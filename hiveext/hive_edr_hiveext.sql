...
ALTER TABLE hive_edr_hiveext SET TBLPROPERTIES('EXTERNAL'='False');
DROP TABLE hive_edr_hiveext;
!set force off

-- we take 21 initial columns, then replicate each one twice (prefix by aa_ and bb_) to make 63; and add 7 partition columns


CREATE EXTERNAL TABLE hive_edr_hiveext
(
...
STORED AS ORC
LOCATION '/data/svc_sqlstream_guavus/hiveext_edr'
TBLPROPERTIES
( "orc.compress" = "SNAPPY"
);
